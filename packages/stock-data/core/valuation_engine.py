...
class ValuationEngine:
    ...
    def _compute_all_factors(cls, symbol: str, window: str) -> Dict[str, Any]:
        ...
        # 3. 场内 ETF
        is_etf = type_str == AssetType.ETF.value or ticker.startswith("51") or ticker.startswith("15")
    ...
    def _compute_nasdaq_valuation(cls, symbol: str, ticker: str, window: str) -> Dict[str, Any]:
        ...
        t = yf.Ticker("QQQ")
        info = t.info
        pe_latest = float(info.get("trailingPE") or 29.30)
        pb_latest = float(info.get("priceToBook") or 2.01)
        div_latest = float(info.get("dividendYield") or 0.42)
        mcap = info.get("totalAssets") or info.get("marketCap") or 280_000_000_000
        ...
        if hist.empty:
            # 降级备选
            p_latest = 718.0
            hist_df = pd.DataFrame({"Close": [p_latest]})
        else:
            p_latest = float(hist["Close"].iloc[-1])
            hist_df = hist.reset_index()

        # 2. 向量化反推时序 PE (Price / EPS_latest)
    ...
    def _compute_sp500_valuation(cls, symbol: str, ticker: str, window: str) -> Dict[str, Any]:
        """标普 500 ETF (513500 / SPY / SPX) 估值与通道计算"""
        t = yf.Ticker("SPY")
        info = t.info
        pe_latest = float(info.get("trailingPE") or 26.50)
        pb_latest = float(info.get("priceToBook") or 4.80)
        div_latest = float(info.get("dividendYield") or 1.25)
        mcap = info.get("totalAssets") or info.get("marketCap") or 550_000_000_000

        hist = t.history(period="5y")
        p_latest = float(hist["Close"].iloc[-1]) if not hist.empty else 550.0
        eps_latest = p_latest / pe_latest if pe_latest > 0 else 1.0
    ...
    def _compute_index_valuation(cls, symbol: str, window: str) -> Dict[str, Any]:
        """指数全量多维分析 (上证红利 / 沪深300 / 中证红利低波)"""
        idx_name = "上证红利"
        if "300" in symbol: idx_name = "沪深300"
        elif "500" in symbol: idx_name = "中证500"
        elif "50" in symbol: idx_name = "上证50"
        elif "深证红利" in symbol or "399324" in symbol: idx_name = "深证红利"
    ...
    def _build_multi_metric_response(
        ...
        # 提取 PE 和 PB 序列
        pe_vals = [float(v) for v in df["pe_ttm"].to_list() if v is not None and not np.isnan(v)] if "pe_ttm" in df.columns else []
        pb_vals = [float(v) for v in df["pb"].to_list() if v is not None and not np.isnan(v)] if "pb" in df.columns else []
