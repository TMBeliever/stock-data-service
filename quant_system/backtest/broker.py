...
from quant_system.core.models import Order, Trade, OrderSide, OrderType, OrderStatus, Portfolio, Position
...
class SimulatedBroker:
    ...
    def match_order(self, order: Order, current_price: float, portfolio: Portfolio, timestamp: int) -> Optional[Trade]:
        ...
        total_buy_cost = exec_price * order.quantity + comm + slip
        ...
        # 卖单可用持仓校验 (T+1 检查)
            ...
            if avail < order.quantity:
                ...
                return None
            # 卖出净收入到账
            sell_proceeds = exec_price * order.quantity - comm - tax - slip
